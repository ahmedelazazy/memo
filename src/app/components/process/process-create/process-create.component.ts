...
export class ProcessCreateComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private templateService: TemplateService,
    private processService: ProcessService, private authService: AuthService, private toastrService: ToastrService, private router: Router) {
  ...
  addProcess(f, templateId) {
    ...
    this.process.user_id = this.authService.user.id;
    this.process.template_id = templateId;
